...
from website import app, db, bcrypt, photos
import secrets
import os
from .form import StaffRegistrationForm, LoginForm, ChangePasswordForm, RoleForm
...
from website.customers.model import Customer, Order, OrderDetail
...
def login():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            elif user.role.name == 'accounting':
...
def update_customer():
    ...
    form = CustomerRegisterForm()
    form.submit.label.text = 'Update Customer'
    # For GET requests, populate form with existing data
    if request.method == 'GET':
    ...
    if request.method == 'POST' and form.validate():
        ...
        db.session.commit()
        flash(f'Customer {form.username.data} updated successfully!', 'success')
        return redirect(url_for('customers_manager'))

    return render_template('customers/register.html', form=form, title='Update Customer Page')
